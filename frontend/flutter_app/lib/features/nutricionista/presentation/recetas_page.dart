        ...
        if (_permitidas != null)
          Card(
            child: Padding(
              ...
              child: SelectableText(const JsonEncoder.withIndent("  ").convert(_permitidas)),
            ),
          ),
        const SizedBox(height: 8),
        Text("Repositorio de recetas", style: Theme.of(context).textTheme.titleMedium),
        const SizedBox(height: 8),
        for (final receta in _recetas)
          Card(
            child: ListTile(
              title: Text(receta["nombre"]?.toString() ?? ""),
              subtitle: Text("id=${receta["id"]} | kcal=${receta["calorias_totales"]}"),
            ),
          ),
      ],
    );

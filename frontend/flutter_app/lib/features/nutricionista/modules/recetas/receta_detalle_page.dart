...
class _RecetaDetallePageState extends ConsumerState<RecetaDetallePage>
  ...
  Widget _buildNutriTable(String title, List<dynamic> rows) {
    return Column(
      ...
      children: [
        if (rows.isNotEmpty) ...[
          Text(_textoLimpio(title).toUpperCase(),
              style: GoogleFonts.montserrat(
                  fontSize: 10,
                  fontWeight: FontWeight.w800,
                  color: Colors.blueGrey,
  ...
  Widget _buildTagChip(Map<String, dynamic> e) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isAlerta ? const Color(0xFFF6C453) : const Color(0xFFE2E8F0),
        ),
        boxShadow: [
          BoxShadow(
              color: Colors.black.withOpacity(0.03),
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            _textoLimpio(e['titulo'] ?? e['nombre_visible'] ?? e['nombre']),
            style: GoogleFonts.montserrat(
              fontSize: 12,
              fontWeight: FontWeight.w700,
              color: isAlerta ? const Color(0xFF7A5200) : AppTema.azulOscuro,
            ),
          ),
        ],
      ),

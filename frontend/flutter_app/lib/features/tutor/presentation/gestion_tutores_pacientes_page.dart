    ...
    return Container(
      ...
      child: TabBar(
        ...
        tabs: const [
          ...
          Tab(icon: Icon(Icons.link_rounded, size: 20), text: "VINCULAR CUENTAS"),
        ],
      ),
    );
  }
...
class _FormRegistroPacienteState extends ConsumerState<_FormRegistroPaciente> {
  ...
  Future<void> _load() async {
    try {
      final s = await ref.read(supabaseCrudRepositoryProvider).fetchCatalog("usuarios", "catalogo_sexo");
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _buildField("Peso (kg)", _pesoController, Icons.monitor_weight_outlined)),
                const SizedBox(width: 16),
                Expanded(child: _buildField("Talla (cm)", _tallaController, Icons.height_rounded)),
              ],
            ),
            const SizedBox(height: 32),
            SizedBox(
    ...
    return Column(
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            prefixIcon: const Icon(Icons.calendar_today_rounded, color: AppTema.azulPrincipal, size: 20),
            filled: true, fillColor: AppTema.grisLienzo.withOpacity(0.5),
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
          ),
        ),
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<int>(
          ...
          decoration: InputDecoration(
            prefixIcon: const Icon(Icons.person_search_rounded, color: AppTema.azulPrincipal, size: 20),
            filled: true, fillColor: AppTema.grisLienzo.withOpacity(0.5),
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
          ),
        ),
...
class _FormVincularTutorPacienteState extends ConsumerState<_FormVincularTutorPaciente> {
  ...
  Future<void> _load() async {
    try {
      final p = await ref.read(supabaseCrudRepositoryProvider).fetchCatalog("usuarios", "parentesco");
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<int>(
          ...
          decoration: InputDecoration(
            prefixIcon: const Icon(Icons.family_restroom_rounded, color: AppTema.azulPrincipal, size: 20),
            filled: true, fillColor: AppTema.grisLienzo.withOpacity(0.5),
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
          ),
        ),
  ...
  Future<void> _vincular() async {
    if (_idTutor == null || _idPaciente == null) {
      NutriSnack.show(context, "Debe seleccionar tutor y paciente", isError: true, ref: ref);
      return;
    }
    setState(() => _loading = true);
    try {
      ...
      if (mounted) NutriSnack.show(context, "Vínculo creado con éxito", ref: ref);
    } catch (e) {
      if (mounted) NutriSnack.show(context, "Error: $e", isError: true, ref: ref);
    } finally {
      if (mounted) setState(() => _loading = false);

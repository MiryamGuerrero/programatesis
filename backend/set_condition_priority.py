...
print("=== CONFIGURANDO JERARQUÍA DE CONDICIONES ===\n")
...
cur.execute("""
    ...
    WHERE id = ANY(%s)
""", (clinical_conditions,))
print(f"   ✅ CLÍNICAS (Artritis, Lupus): orden_oms = 1 (MÁXIMA prioridad)")
...
""")
print(f"   ✅ TEMPORALES (Náuseas, Diarrea): orden_oms = 3 (MÍNIMA prioridad)")
...
""")
print(f"   ✅ NUTRICIONALES (Bajo peso, Obesidad): orden_oms = 2 (MEDIA prioridad)")
...
print("""
   PACIENTE con: Artritis Idiopática Juvenil + Bajo peso
   ...
   PASO 2: Aplicar reglas NUTRICIONALES (orden_oms=2) después:
      - Si NUTRICIONAL dice ELIMINAR omega 3 → SE IGNORA (Artritis ganó)
      - Si NUTRICIONAL dice PRIORIZAR proteína → SE APLICA (no hay choque)
   
   RESULTADO: CLÍNICA > NUTRICIONAL
""")
